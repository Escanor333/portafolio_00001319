...
int ProfundidaddeArbol(Arbol p) {
	int contador1 = 0, contador2 = 0;

	if (p->izq != NULL) {
		contador1 = ProfundidaddeArbol(p->izq);
		contador1++;
	}
	if (p->der != NULL){
		contador2 = ProfundidaddeArbol(p->der);
		contador2++;
	}
...
int main() {
	...
	do {
		...
		case 4: continuar = false;
			break;
		case 3:
			cout << "La Profundidad del Arbol es: " << ProfundidaddeArbol(arbol) << endl;
			break;
